...
export class Player extends Entity {
    ...
    constructor(arena, x, y, r, camera) {
        super(arena, x, y, r, 0);
        this.owner = camera;
        this.health = new COMPONENTS.HealthComponent(100,100,-1);
        ...
        this.petalCooldowns = new Int32Array(40);
        this.damage = 25;
    }
    getAdjustedEquipped() {
        ...
        for (let n = 0; n < this.owner.numEquipped; n++) {
            ...
            if (equipped[n * 2]) {
                this.equipped[n].push({
                    ...
                    petal: null,
                    cdTick: 0,
                    cooldown: PETAL_DEFINITIONS[equipped[n]].cooldown
                });
                this.numSpacesAlloc++;
            } else {
                this.equipped[n] = [{
    ...
    petalHandle() {
        ...
        for (let outer = 0; outer < this.owner.numEquipped; outer++) {
            ...
            for (let inner = 0; inner < innerLength; inner++) {
                ...
                else if (!petal.petal) petal.petal = this._arena.add(new Petal(this._arena, this, this.pos.x, this.pos.y, outer, inner, rotPos, this.owner.equipped[2*outer+1], PETAL_DEFINITIONS[petal.id]))
    ...
    tick() {
        ...
        for (const ent of collisions) {
            ...
            if (ent instanceof Drop) {
                if (this.pos.distanceSq(ent.pos) > (this.collectionRadius) ** 2) continue;
