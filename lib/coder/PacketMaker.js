...
function compileEnt(w, entity, cameraState) {
    if (entity.isDeleted)
        return;
    if ((entity.state & 2) || (cameraState & 2))
        return compileCreate(w, entity);
    else if (entity.state & 1)
        return compileUpdate(w, entity);
}
exports.compileEnt = compileEnt;
function compileInventory(w, inventory, cameraState) {
...
function compileUpdate(w, entity) {
    w.i32(entity.id);
    w.u8(1);
    if (entity.pos) {
    ...
    if (entity.playerInfo) {
        ...
        if (entity.playerInfo.state[1] & 1) {
            w.u8(20);
        ...
        if (entity.playerInfo.state[2] & 1) {
            w.u8(21);
            for (const v of entity.playerInfo.petalHealths.values)
                w.u8(v);
        }
        if (entity.playerInfo.state[3] & 1) {
            w.u8(22);
            for (const v of entity.playerInfo.petalCooldowns.values)
                w.u8(v);
        }
