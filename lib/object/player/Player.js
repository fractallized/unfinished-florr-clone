...
const PetalDefinitions_1 = __importDefault(require("../../consts/PetalDefinitions"));
...
class Player extends Entity_1.default {
        ...
        for (let n = 0; n < repeat; n++) {
            if (definition.noSpawn) {
                this.equipped[pos].push({
                    ...
                    cooldown: 1,
                    definition,
                    creationTick: this._arena._tick
                });
                continue;
            }
            this.equipped[pos].push({
        ...
        for (let outer = 0; outer < this.playerInfo.numEquipped; ++outer) {
            ...
            for (let inner = 0; inner < innerLength; ++inner) {
                ...
                if (petal.cdTick < petal.cooldown)
                    ...
                    petal.petal = new (petal.definition.petal)(this._arena, this, outer, inner, rotPos, petal.rarity, petal.definition, petal.creationTick);
    ...
    tick() {
        ...
        const x = (input & 1) - ((input >> 2) & 1), y = ((input >> 1) & 1) - ((input >> 3) & 1);
        if (x || y) {
            this.playerInfo.faceFlags &= ~7;
            if (x === 1) {
                if (y === 1)
                    this.playerInfo.faceFlags |= 1;
                else if (y === -1)
                    this.playerInfo.faceFlags |= 7;
                else
                    this.playerInfo.faceFlags |= 0;
            }
            else if (x === 0)
                this.playerInfo.faceFlags |= 4 - 2 * y;
            else {
                if (y === 1)
                    this.playerInfo.faceFlags |= 3;
                else if (y === -1)
                    this.playerInfo.faceFlags |= 5;
                else
                    this.playerInfo.faceFlags |= 4;
            }
        }
        const scale = (x && y ? Math.SQRT1_2 : x || y ? 1 : 0) * 2;
        this.accel.set(x, y);
